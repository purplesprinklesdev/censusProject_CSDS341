...
National AS (
    SELECT
        ...
        AVG(pa.pct_race_native_hawaiian_pacific_islander) AS nat_pa.pct_race_native_hawaiian_pacific_islander,
...
SELECT
    ...
    pp.PUMA,
    (
        ABS(pp.avg_age - n.nat_avg_age) / NULLIF(n.nat_avg_age, 0) +
        ABS(pp.pct_male - n.nat_pct_male) / NULLIF(n.nat_pct_male, 0) +
        ABS(pp.pct_hispanic - n.nat_pct_hispanic) / NULLIF(n.nat_pct_hispanic, 0) +
        ABS(pp.pct_race_white - n.nat_pct_race_white) / NULLIF(n.nat_pct_race_white, 0) +
        ABS(pp.pct_race_black - n.nat_pct_race_black) / NULLIF(n.nat_pct_race_black, 0) +
        ABS(pp.pct_race_asian - n.nat_pct_race_asian) / NULLIF(n.nat_pct_race_asian, 0) +
        ABS(pp.pct_race_american_indian - n.nat_pct_race_american_indian) / NULLIF(n.nat_pct_race_american_indian, 0) +
        ABS(pp.pct_race_alaska_native - n.nat_pct_race_alaska_native) / NULLIF(n.nat_pct_race_alaska_native, 0) +
        ABS(pp.pct_race_native_hawaiian_pacific_islander - n.nat_pct_race_native_hawaiian_pacific_islander) / NULLIF(n.nat_pct_race_native_hawaiian_pacific_islander, 0) +
        ABS(pp.pct_some_other_race - n.nat_pct_some_other_race) / NULLIF(n.nat_pct_some_other_race, 0) +
        ABS(pp.pct_two_or_more_races - n.nat_pct_two_or_more_races) / NULLIF(n.nat_pct_two_or_more_races, 0) +
        ABS(pp.pct_bachelors_or_higher - n.nat_pct_bachelors_or_higher) / NULLIF(n.nat_pct_bachelors_or_higher, 0) +
        ABS(pp.pct_worker_in_family - n.nat_pct_worker_in_family) / NULLIF(n.nat_pct_worker_in_family, 0) +
        ABS(pp.avg_weeks_worked - n.nat_avg_weeks_worked) / NULLIF(n.nat_avg_weeks_worked, 0) +
        ABS(pp.pct_owner_occupied - n.nat_pct_owner_occupied) / NULLIF(n.nat_pct_owner_occupied, 0) +
        ABS(pp.avg_household_income - n.nat_avg_household_income) / NULLIF(n.nat_avg_household_income, 0) +
        ABS(pp.pct_children_present - n.nat_pct_children_present) / NULLIF(n.nat_pct_children_present, 0) +
        ABS(pp.avg_gross_rent_pct_income - n.nat_avg_gross_rent_pct_income) / NULLIF(n.nat_avg_gross_rent_pct_income, 0) +
        ABS(pp.avg_property_taxes - n.nat_avg_property_taxes) / NULLIF(n.nat_avg_property_taxes, 0) +
        ABS(pp.pct_married - n.nat_pct_married) / NULLIF(n.nat_pct_married, 0) +
        ABS(pp.pct_in_labor_force - n.nat_pct_in_labor_force) / NULLIF(n.nat_pct_in_labor_force, 0) +
        ABS(pp.pct_english_only - n.nat_pct_english_only) / NULLIF(n.nat_pct_english_only, 0) +
        ABS(pp.pct_english_well - n.nat_pct_english_well) / NULLIF(n.nat_pct_english_well, 0) +
        ABS(pp.pct_immigrant - n.nat_pct_immigrant) / NULLIF(n.nat_pct_immigrant, 0) +
        ABS(pp.pct_health_insured - n.nat_pct_health_insured) / NULLIF(n.nat_pct_health_insured, 0) +
        ABS(pp.pct_citizen - n.nat_pct_citizen) / NULLIF(n.nat_pct_citizen, 0)
    ) AS bellwether_score
FROM PumaProfile pp
